...
function Checkbox({ className, ...props }: CheckboxPrimitive.Root.Props) {
  return (
    <CheckboxPrimitive.Root
      data-slot="checkbox"
      className={cn(
        "peer after:absolute relative after:-inset-x-3 after:-inset-y-2 flex justify-center items-center data-checked:bg-primary dark:bg-input/30 dark:data-checked:bg-primary disabled:opacity-50 group-has-disabled/field:opacity-50 shadow-xs border border-input data-checked:border-primary aria-invalid:aria-checked:border-primary aria-invalid:border-destructive aria-invalid:focus-visible:border-destructive focus-visible:border-ring dark:aria-invalid:border-destructive/50 rounded-[4px] outline-none aria-invalid:focus-visible:ring-destructive/50 focus-visible:ring-[3px] focus-visible:ring-ring/50 size-4 data-checked:text-primary-foreground transition-shadow disabled:cursor-not-allowed shrink-0",
        className,
      )}
      {...props}
    >
